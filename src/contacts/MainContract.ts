...
export function mainContactConfigToCell(config: MainContractConfig): Cell {
  return beginCell()
...
export class MainContract implements Contract {
  constructor(readonly address: Address, readonly init?: {code:  Cell, data: Cell}) {
    console.log('MainContract constructor');
  }

  static createFromConfig(config: MainContractConfig, code: Cell, workchain = 0) {
    const data = mainContactConfigToCell(config);
  ...
  async sendDeploy(provider: ContractProvider, via: Sender, value: bigint) {
    await provider.internal(via, {value, sendMode: SendMode.PAY_GAS_SEPARATELY, body: beginCell().storeUint(2, 32).endCell()});
  }
  ...
  async sendIncrement(provider: ContractProvider, sender: Sender, value: bigint, increment_by: number) {
    ...
    await provider.internal(sender, {value, sendMode: SendMode.PAY_GAS_SEPARATELY,
      body: msg_body});
  }

  async sendDeposit(provider: ContractProvider, sender: Sender, amount: bigint) {
    const msg_body = beginCell().storeUint(2, 32).endCell();
    await provider.internal(sender, {value: amount, sendMode: SendMode.PAY_GAS_SEPARATELY, body: msg_body});
  }
